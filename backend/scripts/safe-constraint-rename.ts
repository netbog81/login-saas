// scripts/safe-constraint-rename.ts
import { AppDataSource } from '../src/data-source';

async function safeConstraintRename() {
  const queryRunner = AppDataSource.createQueryRunner();
  await queryRunner.connect();
  
  try {
    console.log('🛡️  STARTING SAFE CONSTRAINT RENAME IN TRANSACTION...\n');
    console.log('📝 This operation is reversible and safe');
    console.log('📝 Only constraint names will change\n');
    ...
    for (const rename of renames) {
      try {
        if (rename.type === 'FK') {
          await queryRunner.query(`ALTER TABLE ${rename.old.split('_')[1]} RENAME CONSTRAINT "${rename.old}" TO "${rename.new}"`);
        } else if (rename.type === 'IDX') {
          await queryRunner.query(`ALTER INDEX "${rename.old}" RENAME TO "${rename.new}"`);
        ...
        if (error.message.includes('does not exist') || error.message.includes('already exists')) {
          ...
          throw error; // Rilancia errori seri
        }
      }
    }
    ...
    console.log(`\n🔍 Run 'npm run schema:log' to verify the fix`);
    
  } catch (error) {
    await queryRunner.rollbackTransaction();
    console.log('\n�️  ERROR - ALL CHANGES HAVE BEEN ROLLED BACK AUTOMATICALLY!');
    console.log('   Your database is exactly as before the operation');
...
console.log('🛡️  SAFE CONSTRAINT RENAME SCRIPT');
console.log('   ✅ Database backup confirmed');
console.log('   ✅ Transaction protection enabled');
console.log('   ✅ Automatic rollback on any error');
console.log('   Starting in 3 seconds...\n');

setTimeout(() => {
  safeConstraintRename();
}, 3000);
