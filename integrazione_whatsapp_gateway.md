...
CREATE TABLE whatsapp_tenant_config (
  ...
  recap_delay_seconds INT NOT NULL DEFAULT 60,     -- Nel clinico: whatsapp_tenant_config.recapBufferSeconds (30-600s)
...
**Finestra di raggruppamento (`recapDelaySeconds`, 30-600s, default 60):** arriva
da `whatsapp_tenant_config.recapBufferSeconds` ed è **scorrevole**, cioè riparte
a ogni nuovo appuntamento per lo stesso numero — l'operatore non deve chiudere
tutte le prenotazioni entro la prima finestra. Lo slittamento si ferma a
`min(5 × finestra, 15 minuti)` dal primo appuntamento del gruppo.
...
reminder 24h sul nuovo orario. Lato clinico il flag arriva da
`whatsapp_tenant_config.sendUpdateNotification` (default ON).
